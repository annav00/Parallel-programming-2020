...
public static void main(String arr[]) throws Exception{
    ...
    LU.luDecomposition(matrix, matrixB, n, X);
    Instant finish = Instant.now();
    System.out.printf("Параллельный: %d milliseconds%n", Duration.between(start, finish).toMillis());
    int k_max = 1;
    /*for(int n = 50; n < 1500; n += 100){
        ...
        for(int p = n/8; p < n/3; p += n/8){
            ...
            for(int k = 0; k < k_max; k++){
                ...
                start = Instant.now();
                LU_strip.luDecomposition(matrix, matrixB, n, p, X);
                finish = Instant.now();
                //MatrixHelper.printB(X, n, "X:");
                ...
                LU.luDecomposition(matrix, matrixB, n, X);
                finish = Instant.now();
                t3 += Duration.between(start, finish).toMillis();
            }

            System.out.printf("Параллельный: %d milliseconds%n", t1 / k_max);
            System.out.printf("LU для ленточных: %d milliseconds%n", t2 / k_max);
            System.out.printf("LU: %d milliseconds%n", t3 / k_max);
